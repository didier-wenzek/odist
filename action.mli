...
*)
val stream : ('a,'s,'b) Fold.action -> 'a Fold.col -> 'b

(* Printing action of a stream of strings on standard output. *)
val to_printer : (string, unit, unit) Fold.action

(* Printing action of a stream of strings into a file. *)
val to_file_printer : string -> (string, out_channel, unit) Fold.action
...
*)
val to_string_buffer : int -> (string, Buffer.t, string) Fold.action
